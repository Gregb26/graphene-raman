...
ap.add_argument("--v", type=float, default=5.464); ap.add_argument("--gamma-G", type=float, default=1.3346); ap.add_argument("--gamma-K", type=float, default=2.6448); ap.add_argument("--E_D", type=float, default=-4.2389)
...
out = {}
for iq, name, modes, ring, nval, gepw in ((1, "E2g, q = Gamma", (5, 6), range(1, 13), 2, a.gamma_G), (2, "A1', q = K", (3,), range(13, 25), 1, a.gamma_K)):
...
np.savez("results/epw/ring_check_24k24q.npz", **{k.replace(", ", "_").replace(" = ", "").replace("'", "p"): np.array(vv) for k, vv in out.items()}, v=v, A_c=A_c, units="hw eV; S, inter eV^2; ImPi eV (HWHM); gamma_epw meV")
print("saved results/epw/ring_check_24k24q.npz")
